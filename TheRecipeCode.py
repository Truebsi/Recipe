...
def show_filtered_recipes():
    ...
    if not filtered_recipes.empty:
        ...
        # Add a button to return to the overview
...
# In der main() Funktion, ersetze die Zeile, wo show_filtered_recipes() aufgerufen wird, mit:
if not st.session_state.get('show_overview', False):
...
def main():
    ...
    if st.session_state.get('showNewRecipeForm', False):
        ...
        if st.session_state['show_overview']:
            show_recipe_overview()
        else:
            show_filtered_recipes()

        if st.button("Alle Rezepte anzeigen"):
