...
def show_filtered_recipes():
    ...
    if filtered_recipes.empty == False:
        ...
        for index, row in filtered_recipes.iloc[start:end].iterrows():
            ...
            ingredient = zeilenschaltung_zu_html(row['zutaten']) # row['zutaten']
            instructions = zeilenschaltung_zu_html(row['anleitung']) # row['anleitung']
...
def show_random_recipe():
    recipes = st.session_state.recipes
    if recipes.empty:
        st.write("Keine Rezepte verfügbar.")
        return

    random_recipe = recipes.sample(n=1).iloc[0]

    title = random_recipe['rezeptname']
    ingredient = zeilenschaltung_zu_html(random_recipe['zutaten'])
    instructions = zeilenschaltung_zu_html(random_recipe['anleitung'])

    st.markdown(f"<h3>{title}</h3>", unsafe_allow_html=True)
    st.write("Kategorien: ", categories_for_recipe(random_recipe))
    st.markdown(f"Zutaten: {ingredient}", unsafe_allow_html=True)
    st.markdown(f"Anleitung: {instructions}", unsafe_allow_html=True)
    st.write("Link: ", random_recipe['link'])

    if pd.isna(random_recipe['imageurl']):
        st.write(f"Rezept '{random_recipe['rezeptname']}' hat kein Bild.")
    else:
        if os.path.isfile(random_recipe['imageurl']):
            st.image(random_recipe['imageurl'])
        else:
            if st.session_state.github.file_exists(random_recipe['imageurl']):
                image_bytes, _ = st.session_state.github.read(random_recipe['imageurl'])
                st.image(image_bytes)
            else:
                st.write(f"Rezept '{random_recipe['rezeptname']}' hat kein Bild.")
...
def main():
    ...
    if st.session_state.get('showNewRecipeForm', False):
        ...
        if st.button("Was koche ich heute?"):
            st.session_state.show_random_recipe = True
            st.session_state.showNewRecipeForm = False

        if st.session_state.get('show_random_recipe', False):
            show_random_recipe()
        else:
            show_filtered_recipes()
